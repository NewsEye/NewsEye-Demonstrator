...
class CatalogController < ApplicationController
    ...
    def apply_modify_dataset
        ...
        to_add = params['docs'].map do |doc_id|
            ...
            {id: doc_id, type: "compound", relevancy: params['target_relevancy'].to_i, parts: CompoundArticle.find(doc_id).parts}
            end
        end
        d.add_docs to_add
        respond_to do |format|
        ...
        if doc_index.nil?
            if @new_relevancy != -1
                if doc_type == "compound"
                    parts = CompoundArticle.find(@doc_id).parts
                    current_dataset.documents << {id: @doc_id, type: doc_type, relevancy: @new_relevancy, parts: parts}
                    current_dataset.documents.delete_if { |doc| parts.include? doc["id"] }
                else
