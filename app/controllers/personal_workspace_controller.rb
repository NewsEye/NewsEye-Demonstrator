class PersonalWorkspaceController < ApplicationController
    ...
    def update_experiments
        Experiment.where(user: current_user).each do |experiment|
            if experiment.task.nil?
                experiment.run
            else
                ...
                experiment.update(description: Experiment.process_pra_output(data, @current_user, experiment))
            end
        end
            @status = "status"
        end
