...
module Blacklight::FacetsHelperBehavior
  ...
  def has_facet_values? fields = facet_field_names, options = {}
    puts fields
    puts facets_from_request(fields)
  ...
  def render_facet_limit(display_facet, options = {})
    ...
    # Sorting facet values (put sunday at the end), problem if there is not every day present
    if options[:locals][:display_facet].name == "day_isi"
      options[:locals][:display_facet].items.insert(6, options[:locals][:display_facet].items.delete_at(0))
      options[:locals][:display_facet].items.reject! { |item| item.blank? }
    end
    render(options)
  end
  ...
  def path_for_facet(facet_field, item)
    ...
    if facet_config.url_method
      ...
      path = controller.search_state_class.new(params, blacklight_config, controller)
      search_action_path(path.add_facet_params_and_redirect(facet_field, item))
    end
  end
  ...
  def render_selected_facet_value(facet_field, item)
    path = controller.search_state_class.new(params, blacklight_config, controller)
    remove_href = search_action_path(path.remove_facet_params(facet_field, item))
    content_tag(:span, class: "facet-label") do
