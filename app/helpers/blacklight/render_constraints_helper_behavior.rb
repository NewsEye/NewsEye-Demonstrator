...
module Blacklight::RenderConstraintsHelperBehavior
  ...
  def render_filter_element(facet, values, path)
    ...
    safe_join(Array(values).map do |val|
      ...
      if facet == "date_created_dtsi"
        ## ATTENTION aux parametre à permit ou non
        p = Blacklight::Parameters.sanitize(params).except(:page, :counter)
        p[:f].delete :date_created_dtsi
        p.delete(:f) if p[:f].empty?
        p.permit!
        remove_url = search_action_path(p)
