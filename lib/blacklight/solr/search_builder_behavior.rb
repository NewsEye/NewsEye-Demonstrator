    ...
    def add_facet_fq_to_solr(solr_parameters)
      ...
      if blacklight_params[:f]
        ...
        f_request_params.each_pair do |facet_field, value_list|
          ...
          final_facet = "#{terms_filter}#{Array(value_list).reject(&:blank?).join(',')}"
          solr_parameters.append_filter_query final_facet
          # facet_values.each { |fv| solr_parameters.append_filter_query fv }
        end
      end
    end
