...
output_dir = '/home/axel/Nextcloud/NewsEye/newseye_samvera/test_data/'
output = []
...
output << np
File.open(output_dir+'data.json', 'w') do |f|
...
# coll = Newspaper.new
# coll.id = 'np1'
# coll.title = record[:object][:title]
# coll.publisher = record[:object][:proxies][0][:dcPublisher][:def]
# coll.language = record[:object][:proxies][0][:dcLanguage][:def]
# coll.datefrom = record[:object][:proxies][0][:dctermsIssued][:def]
# coll.dateto = record[:object][:proxies][0][:dctermsIssued][:def]
